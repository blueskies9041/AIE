...
Player::Player() : Sprite( "resources/graphics/ZeroSpriteSheet.png", 150, 150, 900, 1200, 75, 75, MyWindow)
{
	Animation Jump;
...
void Player::Input() 
{
	...
	if (GLFW_PRESS == glfwGetKey(m_oGameWindow, GLFW_KEY_W))
		m_v3Speed.y -= 2.0f * fDeltaTime * 60.0f ;
		
    if (GLFW_PRESS == glfwGetKey(m_oGameWindow, GLFW_KEY_A))
        m_v3Speed.x -= 2.0f * fDeltaTime * 60.0f;
